...
/**
 * Fetches from the triplestore a list of properties about a subject. These
 * properties need to all be bound to a value or else an empty result is
 * returned.
 *
 * @public
 * @async
 * @function
 * @param {NamedNode} subject - Subject for which to fetch data for.
 ...
 * subject.
 *
 * NOTE: this asks for all optional properties and rejects if any are missing.
 * Is a better implementation possible? Sending out a very specific query is
 * not going to be much faster than using VALUES, and we need to perform a
 * query in any case.
 */
export async function getDataForSubjectMandatoryProperties(
  subject,
  graph,
  properties,
  mode,
  ...
  for (const prop of properties)
    if (!store.has(subject, prop)) return new N3.Store();
  return store;
}
 ...
 * triplestore.
 */
export async function getDataForSubjectOptionalProperties(
  subject,
  graph,
  properties,
  mode,
  ...
  const store = new N3.Store();
  parsedResults.forEach((triple) =>
    store.addQuad(subject, triple.p, triple.o, triple.g),
  );
  return store;
}
...
/**
 * Get all data from the given CONSTRUCT query and return in a store.
 *
 * @public
 * @async
 * @function
 * @param {String} query - The CONSTRUCT query to execute.
 * @param {namedNode} overrideGraph - Graph override for all the quads in the
 * result store.
 * @returns {N3.Store} Store containing all triples that can be generated by
 * the CONSTRUCT query.
 */
export async function getDataFromConstructQuery(query, overrideGraph) {
  ...
  const resultStore = new N3.Store();
  parsedResults.forEach((res) => {
    resultStore.addQuad(res.s, res.p, res.o, overrideGraph);
  });
  return resultStore;
}
